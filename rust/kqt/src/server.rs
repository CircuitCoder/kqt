...
impl Server {
    pub async fn run(self) -> anyhow::Result<!> {
        ...
        transport.initial_mtu(self.initial_outer_mtu);
        transport.max_idle_timeout(Some(
            Duration::from_secs(self.max_idle_timeout as u64).try_into()?,
        ));
        transport.keep_alive_interval(Some(Duration::from_secs(self.keepalive as u64)));
        transport.congestion_controller_factory(Arc::new(BbrConfig::default()));
        if let Some(s) = self.send_buffer {
            transport.datagram_send_buffer_size(s);
        }
        if let Some(s) = self.recv_buffer {
